...
function main()
{
  ...
  if ! make ; then
    log_error "\e[31mFailed to compile main.cpp\e[0m"
    return 1
  fi
  ...
  for test_file in $( ls ${TESTS_DIR}/*.t ) ; do
    ...
    local ans_output=tmp_ans
    if ! ./${answ} < ${test_file} > ${ans_output} ; then
        log_error "\e[31mFailed to run ans\e[0m"
        return 1
    fi

    local file_line_cnt=; file_line_cnt=$(cat ${test_file} | wc -l | sed -e 's/ *//g')
    # local answer_file=${test_file%.*}.a
    # if ! diff -u ${ans_output} ${tmp_output} ; then
  ...
  local benchmark_bin=./bf
  for test_file in $( ls ${TESTS_DIR}/*.t ) ; do
   log_info "\e[32mRunning ${test_file}\e[0m"

   if ! ./${bin} < ${test_file} ; then
     log_error "\e[31mFailed to run ${bin} for ${test_file}.\e[0m"
     return 1
   fi
